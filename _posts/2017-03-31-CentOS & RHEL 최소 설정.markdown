...
- 작업 순서
0. 파티션 분리 및 기본 디렉토리 생성(/engn001, /srcw001 또는 /srch001, /logs001, -DB-/data001)
1. 계정생성 및 리눅스 환경 확인
...
2. hostname 재설정 (hostname, /etc/hosts, /etc/sysconfig/network) : 딱히 정해진 Rule은 없지만 대부분 '회사 + H/W/D + 시스템 + (RIC) + 개발/운영 + 일련번호 + V
- API Service: LGWAPISVCS01
- DB : LGDSVCS01

3. logroatated 설정 (선택사항)
   /etc/logrotate.conf
...
5. Limit 'su' command (/etc/pam.d/su) 주석제거
...
8. ulimit 환경 수정 (/etc/security/limits.conf)
...
9. System 커널 수정 (/etc/sysctl.conf  --> sysctl -p)
--- 일반 미들웨서 서버
##### max file count #####
fs.file-max = 65535
